...
public class CropZProcess implements TapasProcessingIJ {
    ...
    public ImagePlus execute(ImagePlus input) {
        ...
        String key = TapasBatchUtils.getKey(minS, info, getParameter(USERS));
        if (key == null) {
            IJ.log("No key " + minS);
            return null;
        }
        ...
        key = TapasBatchUtils.getKey(maxS, info, getParameter(USERS));
        if (key == null) {
            IJ.log("No key " + maxS);
            return null;
        }
