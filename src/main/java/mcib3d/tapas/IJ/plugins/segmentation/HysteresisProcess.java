...
public class HysteresisProcess implements TapasProcessingIJ {
    ...
    public ImagePlus execute(ImagePlus input) {
        ...
        String key = TapasBatchUtils.getKey(minS, info, users);
        if (key == null) {
            IJ.log("No key " + minS);
            return null;
        }
        ...
        key = TapasBatchUtils.getKey(maxS, info, users);
        if (key == null) {
            IJ.log("No key " + maxS);
            return null;
        }
