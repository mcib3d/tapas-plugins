...
public class FillHoles2D {
    ...
    public static void fill(ImageHandler image, int foreground, int background) {
        if (image.sizeZ == 1) {
    ...
    public static void fill(ImagePlus image, int foreground, int background) {
        if (image.getNSlices() == 1) {
            fill(image.getProcessor(), foreground, background);
        } else {
            ImageStack stack = image.getImageStack();
            for (int i = 1; i <= image.getNSlices(); i++) { // TODO multithread
                fill(stack.getProcessor(i), foreground, background);
            }
        }
    }
